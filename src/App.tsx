...
function AppContent() {
  ...
  // Global Keyboard Shortcuts (ESC to close modals, M for mute, G for grimoire/story, H/? for rules)
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        if (showExitConfirmModal) { setShowExitConfirmModal(false); return; }
        if (inspectCard) { setInspectCard(null); return; }
        if (showAccuseModal) { setShowAccuseModal(false); return; }
        if (showInGameMenu) { setShowInGameMenu(false); return; }
        if (showLocalActionModal) { setShowLocalActionModal(false); return; }
        if (showStoryModal) { setShowStoryModal(false); return; }
        if (showAccusationsModal) { setShowAccusationsModal(false); return; }
        ...
        if (showNotifications) { setShowNotifications(false); return; }
        if (showRules) { setShowRules(false); return; }
        if (showDiscardedModal) { setShowDiscardedModal(false); return; }
        if (showRoleRevealCutscene) { setShowRoleRevealCutscene(false); return; }
        return;
      }
  ...
  useEffect(() => {
    ...
    s.on('error_message', (msg: string) => {
      console.warn('[Aviso do Códice]:', msg);
      setRoomCreationError(msg);
  ...
  // Handler: Quick Host Online Room ("SALA PRIVADA")
  const handleHostRoom = async () => {
    ...
    try {
      ...
      const initial = createNewRoom(code, playerName || 'Investigador Líder', selectedCharId || 'char_oraculo');
      if (!initial || !initial.players || initial.players.length === 0) {
        throw new Error('Falha ao gerar os fundamentos da câmara privada.');
      }

      initial.roomName = 'SALA PRIVADA';
      initial.gameMode = 'PERSONALIZADO';
      initial.settings.maxPlayers = 10;
      initial.settings.hasAccomplice = true;
      initial.settings.allowEvents = true;
      initial.settings.allowAbilities = true;

      // Populate with bots by default so room has investigators ready and can be started immediately
      const withBots = populateLobbyInvestigators(initial, 6);
      const finalInitial = withBots || initial;

      setMyPlayerId(finalInitial.players[0].id);
      setRoom(finalInitial);
      ...
      if (socket && socket.connected) {
        ...
        socket.emit('join_room', {
          ...
          includeBots: true,
          roomSettings: finalInitial.settings,
      ...
      console.error('[Códice] Falha ao criar sala privada:', err);
      const errMsg = err?.message || 'Falha ao criar sala privada. Verifique sua conexão e tente novamente.';
      setRoomCreationError(errMsg);
      setRoom(null);
    } finally {
      setIsCreatingRoom(false);
    }
  ...
  const handleConfirmCreateRoom = async (config: {
    roomName: string;
    ...
    try {
      ...
      const initial = createNewRoom(code, playerName || 'Investigador Líder', selectedCharId || 'char_oraculo');
      if (!initial || !initial.players || initial.players.length === 0) {
        throw new Error('Falha ao gerar os fundamentos da câmara.');
      }

      initial.roomName = config.roomName;
      initial.gameMode = config.gameMode;
      initial.isPrivate = config.isPrivate ?? false;
      ...
      const finalInitial = config.includeBots ? populateLobbyInvestigators(initial, config.maxPlayers || 10) : initial;

      if (!finalInitial || !finalInitial.players || finalInitial.players.length === 0) {
        throw new Error('Falha ao povoar investigadores na câmara.');
      }

      // Immediately render the lobby with initial state to eliminate black screen
      setMyPlayerId(finalInitial.players[0].id);
      setRoom(finalInitial);
      ...
      console.error('[Códice] Falha ao criar sala:', err);
      const errMsg = err?.message || 'Falha ao criar sala. Verifique sua conexão e tente novamente.';
      setRoomCreationError(errMsg);
      // Ensure state is clean and won't crash into a black screen
      setRoom(null);
  ...
  const handleJoinRoom = (customCode?: string, password?: string) => {
  ...
  const handleStartGame = () => {
    ...
    if (socket && socket.connected && !isLocalModeRef.current) {
      if (room.players.length < 4) {
        socket.emit('fill_bots', { targetCount: 4 });
      }
      socket.emit('start_game');
    } else {
      try {
        const readyRoom = room.players.length < 4 ? fillWithAIBots(room, 4) : room;
        const updated = startGameDistribution(readyRoom);
        setRoom(updated);
      } catch (err: any) {
        console.error('Error starting game:', err);
        setRoomCreationError(err?.message || 'Erro ao iniciar partida.');
      }
    }
  };
  ...
  const handleAnswerAnalystInquiryAction = (selectedItem: string) => {
    if (socket && socket.connected && !isLocalModeRef.current) {
      socket.emit('answer_analyst_inquiry', { selectedItem });
    } else if (room) {
      try {
        const updated = handleAnswerAnalystInquiry(room, selectedItem, myPlayerId);
        setRoom(updated);
      } catch (err: any) {
        alert(err.message);
      }
  ...
  const handleSendMessage = (
    ...
    if (socket && socket.connected && !isLocalModeRef.current) {
      ...
      // ChatGPT-style AI Bot personal conversational response in local mode
      if (
        !effectiveIsWhisper &&
        ...
        setTimeout(() => {
          setRoom((currentRoom) => {
            if (!currentRoom || currentRoom.phase !== 'INVESTIGACAO') return currentRoom;
            const personalReply = generateAIBotPersonalResponse(currentRoom, myPlayerId, text || '');
            if (!personalReply) return currentRoom;
            return {
              ...currentRoom,
              messages: [
                ...currentRoom.messages,
                {
                  ...
                  senderId: personalReply.bot.id,
                  senderName: personalReply.bot.name,
                  text: personalReply.replyText,
                  timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
                },
              ],
            };
  ...
  if (!room) {
    return (
      <div
        id="app-root-home"
        className="app-container min-h-[100dvh] font-serif flex flex-col justify-between selection:bg-[#ff4444] selection:text-white relative transition-colors duration-300 bg-cover bg-center"
        style={{ backgroundImage: `url(${codiceMorteLivroImg})` }}
      >
        <AtmosphericDust density={22} />
        {/* Global Error Notification Toast for Room Creation and Server Errors */}
        {roomCreationError && (
          <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[110] max-w-md w-[92%] bg-red-950/95 border-2 border-red-500 rounded-2xl p-3.5 sm:p-4 shadow-[0_0_30px_rgba(239,68,68,0.5)] backdrop-blur-md flex items-center justify-between gap-3 animate-fade-in text-white select-none">
            <div className="flex items-center gap-3 min-w-0">
              <AlertTriangle className="w-5 h-5 text-red-400 shrink-0 animate-pulse" />
              <div className="min-w-0">
                <span className="font-serif font-black text-[10px] text-red-300 uppercase tracking-widest block">
                  Aviso do Códice
                </span>
                <p className="text-xs text-zinc-200 font-serif leading-snug truncate sm:whitespace-normal">
                  {roomCreationError}
                </p>
              </div>
            </div>
            <button
              onClick={() => setRoomCreationError(null)}
              className="p-1.5 rounded-lg bg-black/40 hover:bg-black/70 text-zinc-300 hover:text-white transition-all shrink-0 border border-white/10"
              title="Fechar"
            >
              <X className="w-4 h-4" />
            </button>
        ...
        {isCreatingRoom && (
          <div className="fixed inset-0 z-[120] bg-black/85 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center select-none animate-fade-in">
            <div className="w-16 h-16 rounded-2xl border-2 border-amber-500/60 flex items-center justify-center mb-4 bg-amber-950/40 shadow-[0_0_40px_rgba(245,158,11,0.4)]">
              <Loader2 className="w-8 h-8 animate-spin text-amber-400" />
            </div>
            <h3 className="font-serif font-black text-amber-200 text-base sm:text-lg uppercase tracking-[0.25em] mb-1">
              CONSAGRANDO CÂMARA...
            </h3>
            <p className="text-xs font-serif text-zinc-400 max-w-xs leading-relaxed">
              Iniciando registros ancestrais e adentrando o saguão dos investigadores...
            </p>
          </div>
        )}
...
function AppContent() {
  ...
  // ... mover estados e handlers aqui para AppContent ...
      ...
      <AtmosphericDust density={26} />

      {/* Top Navbar - Hidden during match to prevent covering modal close/back buttons */}
      <header
        style={{ display: 'none' }}
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {myRole && (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1 sm:py-1.5 rounded-xl text-xs font-serif font-bold border shadow-md shrink-0 transition-all hover:scale-105 active:scale-95 cursor-pointer uppercase tracking-wider ${
                myRole === 'oraculo'
          ...
          <>
            {myRole === 'oraculo' ? (
              <OracleView
                ...
                onAnswerAnalystInquiry={handleAnswerAnalystInquiryAction}
