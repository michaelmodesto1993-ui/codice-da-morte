...
import { MARKER_INFOS, METHODS, OBJECTS, EVENTS, ABILITIES, CHARACTERS, EVIDENCES } from '../data/gameData';
...
export const OracleView: React.FC<OracleViewProps> = ({
  ...
  // Auto-select golden marker when interpreter request is active
  useEffect(() => {
    if (room.interpreterRequest?.active || room.activeAbility?.ability.id === 'H10') {
      setSelectedColor('dourado');
    }
  }, [room.interpreterRequest?.active, room.activeAbility?.ability.id]);
  ...
  return (
    <div className="w-full flex flex-col bg-[#0b0504] text-[#e8ded5] rounded-3xl overflow-hidden border-2 border-amber-500/50 shadow-2xl select-none max-w-6xl mx-auto animate-fade-in font-serif">
      ...
      <div className="p-3 sm:p-6 space-y-5 min-h-[480px]">
        {/* INTERPRETER REQUEST BANNER (H10) */}
        {(room.interpreterRequest?.active || room.activeAbility?.ability.id === 'H10') && (
          <div className="p-3.5 sm:p-4 rounded-2xl bg-gradient-to-r from-amber-950 via-[#221503] to-amber-950 border-2 border-amber-400 shadow-[0_0_20px_rgba(251,191,36,0.3)] animate-fade-in flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2.5">
              <Sparkles className="w-6 h-6 text-amber-300 animate-spin shrink-0" />
              <div>
                <h4 className="text-xs sm:text-sm font-serif font-black text-amber-200 uppercase tracking-wider flex items-center gap-2">
                  <span>SOLICITAÇÃO DO INTÉRPRETE ATIVA (H10)</span>
                  <span className="text-[9px] font-mono bg-amber-500/20 text-amber-300 px-2 py-0.5 rounded-full border border-amber-400/40">
                    Selo Dourado Obrigatório
                  </span>
                </h4>
                <p className="text-xs font-serif text-amber-100/90 pt-0.5">
                  {room.interpreterRequest?.requesterPlayerName || 'O Intérprete'} solicitou a colocação de um <strong>Marcador Dourado</strong> na pista central do mistério! O marcador dourado foi pré-selecionado para você.
                </p>
              </div>
            </div>
            <button
              ...
              className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-500 hover:from-amber-400 hover:to-yellow-300 text-black font-serif font-black text-xs uppercase tracking-wider transition-all shadow-lg active:scale-95 shrink-0"
            >
              Ir para Evidências com Selo Dourado
            </button>
          </div>
        )}

        {/* ANALYST INQUIRY BANNER (H04) */}
        {room.analystInquiry && !room.analystInquiry.answered && (
          <div className="p-4 rounded-2xl bg-gradient-to-r from-amber-950 via-[#1c0c05] to-amber-950 border-2 border-amber-400 shadow-[0_0_20px_rgba(245,158,11,0.35)] animate-fade-in space-y-3">
            <div className="flex items-center justify-between gap-2 pb-2 border-b border-amber-500/30">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs sm:text-sm font-serif font-black text-amber-200 uppercase tracking-widest">
                  INQUÉRITO COMPARATIVO DO ANALISTA (H04)
                </h3>
              </div>
              <span className="text-[10px] font-mono text-amber-300 bg-black/60 px-2.5 py-0.5 rounded-full border border-amber-500/50">
                Solicitado por: {room.analystInquiry.analystPlayerName}
              </span>
            </div>
            <p className="text-xs font-serif text-amber-100/90 leading-relaxed">
              O Investigador <strong>{room.analystInquiry.analystPlayerName} [Analista]</strong> realizou uma consulta formal. Como Oráculo da verdade, aponte qual destes 2 itens é <strong>mais verdadeiro</strong> para guiar a investigação:
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
              {room.analystInquiry.items.map((item, idx) => (
                <button
                  ...
                  className="p-3.5 rounded-xl bg-gradient-to-r from-amber-900/90 via-amber-800/90 to-amber-900/90 hover:from-amber-700 hover:to-amber-600 border border-amber-400 text-white font-serif font-bold text-xs sm:text-sm transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2 text-center"
                >
                  <CheckCircle className="w-4 h-4 text-amber-300 shrink-0" />
                  <span>"{item}" é Mais Verdadeiro</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {room.interpreterRequest?.active && (
          <div className="p-4 rounded-2xl bg-gradient-to-r from-amber-950 via-yellow-950/80 to-amber-950 border-2 border-yellow-400 shadow-[0_0_25px_rgba(250,204,21,0.35)] animate-fade-in space-y-2">
            <div className="flex items-center justify-between gap-2 pb-1.5 border-b border-yellow-400/30">
              <div className="flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-yellow-300 animate-spin" />
                <h3 className="text-xs sm:text-sm font-serif font-black text-yellow-100 uppercase tracking-widest">
                  SOLICITAÇÃO DO INTÉRPRETE (H10) • MARCADOR DOURADO
                </h3>
              </div>
              <span className="text-[10px] font-mono text-yellow-200 bg-black/60 px-2.5 py-0.5 rounded-full border border-yellow-400/50">
                Por: {room.interpreterRequest.requesterPlayerName || 'Intérprete'}
              </span>
            </div>
            <p className="text-xs font-serif text-yellow-100/90 leading-relaxed">
              O Investigador <strong>{room.interpreterRequest.requesterPlayerName || 'Intérprete'}</strong> usou sua habilidade de Intérprete solicitando que você, como Oráculo, marque a pista central com o <strong>Marcador Dourado</strong>!
            </p>
          </div>
        )}

        {/* ACTIVE EVENT BANNER IN ORACLE VIEW */}
        {room.activeEvent?.event && (
          <div className="p-3.5 rounded-2xl bg-gradient-to-r from-orange-950/90 via-black to-red-950/90 border border-orange-500/60 shadow-lg flex items-center justify-between gap-3 animate-fade-in">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-orange-950 border border-orange-500/50 flex items-center justify-center shrink-0">
                <Flame className="w-4 h-4 text-orange-400 animate-pulse" />
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-mono text-orange-400 font-bold uppercase tracking-wider">
                    EVENTO ATIVO • RODADA {room.round}
                  </span>
                  <span className="text-[9px] font-mono text-orange-300 bg-black/60 px-2 py-0.2 rounded-full border border-orange-500/40">
                    {room.activeEvent.remainingSeconds}s
                  </span>
                </div>
                <h4 className="font-serif text-xs sm:text-sm font-black text-white">
                  {room.activeEvent.event.name}
                </h4>
                <p className="text-[11px] font-serif text-zinc-300">
                  {room.activeEvent.event.description}
                </p>
              </div>
            </div>
            {room.activeEvent.event.id === 'EV14' && (
              <span className="px-2.5 py-1 rounded-xl bg-indigo-950 border border-indigo-400/50 text-indigo-200 text-[10px] font-serif shrink-0">
                Apenas 3 evidências visíveis a investigadores
              </span>
            )}
          </div>
        )}
          ...
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="rounded-2xl p-4 sm:p-5 bg-gradient-to-b from-[#180b06] via-[#100604] to-[#080201] border border-amber-500/40 shadow-xl relative overflow-hidden">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 pb-3 border-b border-amber-500/20">
                ...
                {/* Zoom Controls */}
                <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-black/80 border border-amber-500/40 text-amber-200 self-end sm:self-auto">
              ...
              <div className="mt-3.5 flex flex-col md:flex-row items-center gap-4">
                <div className="w-full md:w-48 h-36 rounded-xl overflow-hidden border border-amber-500/30 shrink-0 relative bg-black">
                  <img
                    src={libraryHeaderBg}
                  ...
                  <div className="absolute bottom-2 left-2 right-2 flex justify-center">
                    <span className="text-[10px] font-mono text-amber-300 bg-black/80 px-2 py-0.5 rounded border border-amber-500/40">
                      Cena do Crime
                    </span>
                  </div>
                </div>

                <div className="flex-1 space-y-2 text-left w-full overflow-hidden">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm sm:text-base font-serif font-black text-amber-200 uppercase tracking-wider">
                      CAPÍTULO ÚNICO: A CENA DO CRIME
                    </h3>
                    <span className="text-[10px] font-mono text-amber-400/80 px-2 py-0.5 rounded bg-amber-950/60 border border-amber-500/30">
                      Crônica Canônica
                    </span>
                  </div>
                  <div className="overflow-auto max-h-64 sm:max-h-72 p-1">
                    <p
                      style={{
                      ...
                      className="text-[#e0d6c8] font-serif leading-relaxed italic bg-black/50 p-4 rounded-xl border border-white/5 shadow-inner whitespace-pre-line select-text"
                    >
                      "{customStory || room.storyNarrative || DEFAULT_CANONICAL_STORY}"
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-amber-500/20 space-y-2">
                ...
                <textarea
                  ...
                  className="w-full h-24 p-3 bg-black/70 border border-amber-500/30 rounded-xl text-xs sm:text-sm font-serif text-amber-100 placeholder-zinc-500 focus:border-amber-400 focus:outline-none resize-none leading-relaxed"
                />
                <div className="flex flex-col sm:flex-row items-center justify-between gap-2 pt-1">
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    ...
                    <button
                      ...
                      onClick={() => {
                        soundEngine.playClick();
                        setCustomStory(DEFAULT_CANONICAL_STORY);
                        if (onUpdateStory) {
                          onUpdateStory(DEFAULT_CANONICAL_STORY);
                        }
                        setSavedNarrativeAlert(true);
                        setTimeout(() => setSavedNarrativeAlert(false), 2500);
                      }}
                      className="flex-1 sm:flex-none px-4 py-1.5 rounded-xl bg-gradient-to-r from-red-950 to-red-900 hover:from-red-900 hover:to-red-800 text-amber-200 font-serif font-bold text-xs uppercase tracking-wider transition-all shadow-md active:scale-95 border border-red-500/40 flex items-center justify-center gap-1.5"
                    >
                      <RotateCcw className="w-3.5 h-3.5 text-amber-400" />
                      <span>Restaurar Canônica</span>
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-6 py-2">
              ...
              <div className="flex-1 flex flex-col justify-between space-y-3 w-full bg-black/60 p-4 sm:p-5 rounded-2xl border border-amber-900/40">
                <div>
                  ...
                  {(inspectItem.item as any)?.category && (
                    <span className="text-xs font-serif text-amber-400/90 font-bold block mt-0.5">
                      Categoria: {(inspectItem.item as any).category}
                    </span>
                  )}

                  {(inspectItem as any).isSolution && (
                  ...
                  {(inspectItem.item as any)?.description && (
                    <p className="text-xs sm:text-sm text-zinc-300 font-serif leading-relaxed mt-2.5 italic">
                      "{(inspectItem.item as any).description}"
                    </p>
                  )}

                  {(inspectItem.item as any)?.bio && (
                    <p className="text-xs sm:text-sm text-zinc-300 font-serif leading-relaxed mt-2.5 italic">
                      "{(inspectItem.item as any).bio}"
                    </p>
                  )}

                  {(inspectItem.item as any)?.lore && (
                    <p className="text-xs sm:text-sm text-zinc-300 font-serif leading-relaxed mt-2.5 italic">
                      "{(inspectItem.item as any).lore}"
                    </p>
                  )}
