...
export const CharacterToken2D: React.FC<CharacterToken2DProps> = ({
  ...
  return (
    <div
      ...
      className={`group relative flex flex-col items-center cursor-pointer transition-all duration-300 transform select-none hover:-translate-y-1.5 focus:outline-none shrink w-[68px] sm:w-[92px] md:w-[105px] max-w-[110px] ${
    ...
    >
      ...
      <div className="flex flex-col items-center mb-0.5 sm:mb-1 z-10 w-full px-0.5">
        <div className="flex items-center gap-0.5 sm:gap-1 flex-wrap justify-center">
          <span className="text-[7.5px] sm:text-[9px] font-mono font-bold text-amber-300 bg-black/90 border border-amber-500/50 px-1 py-0.2 rounded shadow-md backdrop-blur-xs">
            {displayNumber}
          </span>
          {isMe && (
            <span className="text-[6.5px] sm:text-[8px] font-mono font-bold text-amber-200 bg-amber-600/90 px-1 rounded shadow">
              VOCÊ
            </span>
          )}
          {isOracle && (
            <span className="text-[6.5px] sm:text-[8px] font-mono font-bold text-purple-200 bg-purple-900/90 border border-purple-400/60 px-1 rounded shadow animate-pulse">
              👑 ORÁCULO
            </span>
          )}
          {isAssassin && (
            <span className="text-[6.5px] sm:text-[8px] font-mono font-bold text-red-100 bg-red-950/95 border border-red-500/80 px-1 rounded shadow animate-pulse">
              🔪 ASSASSINO
            </span>
          )}
          {isAccomplice && (
            <span className="text-[6.5px] sm:text-[8px] font-mono font-bold text-violet-100 bg-violet-950/95 border border-violet-500/80 px-1 rounded shadow">
              👁️ CÚMPLICE
            </span>
          )}
          {isSaboteur && (
            <span className="text-[6.5px] sm:text-[8px] font-mono font-bold text-amber-100 bg-amber-950/95 border border-amber-500/80 px-1 rounded shadow">
              💣 SABOTADOR
            </span>
          )}
        </div>
        <span className="text-[9px] sm:text-xs font-serif font-bold text-zinc-100 tracking-wider group-hover:text-amber-300 transition-colors drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)] text-center w-full truncate mt-0.5">
          {player.name || char?.name}
        </span>
        <span className={`text-[7.5px] sm:text-[9px] font-serif font-semibold tracking-wide ${roleTextClass} drop-shadow-[0_1px_3px_rgba(0,0,0,0.9)] text-center truncate w-full`}>
          {isOracle ? '🔮 Guia do Crime' : roleName}
        </span>
      </div>
      ...
      <div className="relative flex flex-col items-center">
        {/* Ambient Ring / Halo */}
        <div
          className={`w-11 h-14 sm:w-16 sm:h-20 md:w-20 md:h-24 rounded-xl sm:rounded-2xl overflow-hidden relative border transition-all duration-300 shadow-2xl ${
            isOracle
