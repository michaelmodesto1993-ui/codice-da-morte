...
export const CrimeNarrativeModal: React.FC<CrimeNarrativeModalProps> = ({
  ...
  const defaultCrimeStory = room.storyNarrative ||
  ...
  const [editedText, setEditedText] = useState(defaultCrimeStory);
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <div className="flex items-center gap-2 text-amber-400/80 mb-4 pb-2 border-b border-amber-500/20">
                ...
                <span className="text-[11px] font-mono uppercase tracking-[0.25em] font-bold">
                  Crônica Oficial do Códice • Capítulo Único
                </span>
              </div>
              ...
              <p className="text-sm sm:text-base text-zinc-100 font-serif italic leading-relaxed tracking-wide selection:bg-amber-900 selection:text-white whitespace-pre-line">
                "{room.storyNarrative || defaultCrimeStory}"
              </p>
            ...
            {isOracle && (
              <div className="p-4 rounded-2xl glass-ui-amber border-amber-400/40 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                ...
                <button
                  onClick={() => {
                    setEditedText(room.storyNarrative || defaultCrimeStory);
                    setIsEditing(true);
                  }}
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <textarea
                ...
                rows={7}
                placeholder="Escreva a narrativa investigativa do crime..."
            ...
            <div className="space-y-2">
              <span className="text-[11px] font-mono text-zinc-400 block uppercase tracking-wider">
                Modelos de Investigação Dedutiva:
              </span>
              <div className="flex flex-wrap gap-2">
                <button
                  type="button"
                  onClick={() =>
                    handleApplyPreset(defaultCrimeStory)
                  }
                  className="px-3 py-1.5 rounded-xl text-[11px] font-serif bg-amber-950/50 hover:bg-amber-900/60 border border-amber-500/50 hover:border-amber-400 text-amber-200 transition-all text-left"
                >
                  🔍 Modelo Canônico (Pistas Sutis)
                </button>
                <button
                  type="button"
                  onClick={() =>
                    handleApplyPreset(
                      `Sob o rugido dos trovões que estremeciam os vitrais da biblioteca, a calada da noite foi rompida pela descoberta do corpo inerte. As sombras da abadia guardam o enigma da traição, e as gemas do Oráculo são o único farol para que os detetives desvendem o mistério antes do amanhecer.`
                    )
                  }
                ...
                <button
                  ...
                  onClick={() =>
                    handleApplyPreset(
                      `No corredor dos arquivos antigos, a brisa gélida apagou as tochas das paredes. Pegadas fugazes desapareceram na direção da ala leste, onde as tapeçarias empoeiradas abafam os segredos de linhagens esquecidas. O mistério exige astúcia de quem ousa decifrar os sinais.`
                    )
                  }
                  className="px-3 py-1.5 rounded-xl text-[11px] font-serif bg-black/50 hover:bg-amber-950/60 border border-white/10 hover:border-amber-400/40 text-zinc-300 transition-all text-left"
                >
                  📜 Segredos do Claustro
                </button>
              </div>
            </div>
