...
import { InvestigatorNotebookModal } from './InvestigatorNotebookModal';
...
interface InvestigationRoomViewProps {
  ...
  onAnswerAnalystInquiry?: (selectedItem: string) => void;
}
...
export const InvestigationRoomView: React.FC<InvestigationRoomViewProps> = ({
  ...
  // Chat Modal states
  const [chatTab, setChatTab] = useState<'PUBLICO' | 'SUSSURRO' | 'NOTAS'>('PUBLICO');
  const [chatInputText, setChatInputText] = useState<string>('');

  // Investigator In-Chat Notebook State & Local Persistence
  const [chatNotesText, setChatNotesText] = useState<string>('');
  const [chatNotesSavedNotice, setChatNotesSavedNotice] = useState<boolean>(false);
  const notebookStorageKey = `gothic_notebook_${room.code || 'default'}_${myPlayerId}`;

  // Load persistent notebook notes
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (parsed.generalNotes !== undefined) {
          setChatNotesText(parsed.generalNotes);
        }
      }
    } catch (e) {
      console.warn('Erro ao carregar anotações locais do investigador:', e);
    }
  }, [notebookStorageKey, activeFloatingModal]);

  const handleUpdateChatNotes = (val: string) => {
    setChatNotesText(val);
    try {
      const saved = localStorage.getItem(notebookStorageKey);
      const parsed = saved ? JSON.parse(saved) : {};
      parsed.generalNotes = val;
      parsed.updatedAt = new Date().toISOString();
      localStorage.setItem(notebookStorageKey, JSON.stringify(parsed));
      setChatNotesSavedNotice(true);
      setTimeout(() => setChatNotesSavedNotice(false), 2000);
    } catch (e) {
      console.error('Falha ao salvar anotações do investigador:', e);
    }
  };
  ...
  // Story excerpt - Canonical investigative crime chronicle
  ...
  useEffect(() => {
    ...
    const unsubParticipants = voiceManager.onParticipantsChange((map) => {
      const active: string[] = [];
      map.forEach((p, pId) => {
        if (p.isSpeaking && pId !== myPlayerId) {
          const found = room.players.find((pl) => pl.id === pId);
          if (found) active.push(found.name);
        }
      });
  ...
  return (
    <div
      ...
      className="app-container relative w-full flex flex-col bg-transparent text-[#e0d8d0] select-none gothic-floating-layout space-y-3.5 overflow-x-auto max-w-full"
    >
      {/* ---------------------------------------------------- */}
      ...
      <div className="relative z-20 py-2.5 px-4 flex flex-col items-center justify-center text-center rounded-2xl bg-black/75 backdrop-blur-xl shadow-lg border border-amber-500/20">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 mt-1.5">
          ...
          {/* Discord-style Live Speaking Indicator for Remote Peers */}
          {speakingPeers.length > 0 && (
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-emerald-950/90 border border-emerald-400 text-emerald-300 text-[10px] font-mono shadow-md animate-pulse">
              <Radio className="w-3.5 h-3.5 text-emerald-400 animate-spin" />
              <span className="truncate max-w-[150px]">{speakingPeers.join(', ')} falando...</span>
            </div>
          )}
      ...
      >
        ...
        <div className="relative flex-1 min-h-[380px] sm:min-h-[460px] flex flex-col justify-between overflow-hidden">
          ...
          <div className="relative z-20 px-2 sm:px-4 pt-2 flex items-center justify-between gap-2 pointer-events-auto overflow-x-auto no-scrollbar scrollbar-none max-w-full pb-1">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {onOpenMenu && (
                <button
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-gradient-to-r from-amber-700/80 to-[#240e06] hover:from-amber-600 hover:to-[#38160a] border border-amber-400/60 text-amber-100 text-[10px] font-serif font-black tracking-wider uppercase shadow-md transition-all active:scale-95 backdrop-blur-xs shrink-0"
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-black/75 hover:bg-amber-950/80 border border-amber-500/40 hover:border-amber-400 text-amber-200 text-[10px] font-serif font-bold tracking-wider shadow-md transition-all active:scale-95 backdrop-blur-xs shrink-0"
            ...
            {/* Right: Notes & Fullscreen 2D Table Buttons */}
            <div className="flex items-center gap-1.5 shrink-0">
              <button
                onClick={() => {
                  soundEngine.playClick();
                ...
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-gradient-to-r from-amber-950/90 to-[#241706] hover:from-amber-900 border border-amber-500/50 text-amber-200 text-[10px] font-serif font-black tracking-wider uppercase shadow-md transition-all active:scale-95 backdrop-blur-xs group cursor-pointer shrink-0"
                title="Abrir Caderno de Anotações do Investigador"
              >
                <NotebookPen className="w-3.5 h-3.5 text-amber-300 group-hover:scale-110 transition-transform" />
                <span>NOTAS</span>
              </button>
          ...
          <div
            ...
            className="relative z-10 w-full flex-1 py-2 sm:py-4 px-2 sm:px-4 flex flex-col justify-around gap-2 sm:gap-3 overflow-y-auto overflow-x-auto no-scrollbar max-h-[500px]"
          >
            <div
              style={{
            ...
            >
              {(() => {
                ...
                // Mobile portrait friendly layout: max 2-3 players per row so all players fit on screen without horizontal clipping
                let rows: Player[][] = [];
                if (total <= 3) {
                  rows = [players];
                } else if (total === 4) {
                  rows = [players.slice(0, 2), players.slice(2)];
                } else if (total <= 6) {
                  rows = [players.slice(0, 3), players.slice(3)];
                } else if (total <= 8) {
                  // 3 rows: 3, 3, 2
                  rows = [
                    players.slice(0, 3),
                  ...
                  // 9-12 players: max 3 per row for clean mobile layout
                ...
                return rows.map((rowPlayers, rIdx) => (
                  <div
                    ...
                    className="flex items-center justify-center gap-1 sm:gap-3 flex-wrap sm:flex-nowrap w-full px-0.5 sm:px-2 py-0.5"
                  >
      ...
      {/* ---------------------------------------------------- */}
      {/* ACTIVE EVENT / ABILITY / INQUIRY BANNER ALERTS */}
      {/* ---------------------------------------------------- */}
      {/* 1. Active Event Alert Banner (e.g. Sussurro, Luz Fraca, Tempestade, etc.) */}
      {room.activeEvent?.event && (
        <section className="relative z-20 px-4 py-3 rounded-2xl bg-gradient-to-r from-orange-950/90 via-black/90 to-red-950/90 border border-orange-500/60 shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-fade-in">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-orange-950 border border-orange-500/50 flex items-center justify-center shrink-0 shadow-md">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] font-mono text-orange-300 bg-black/60 px-2 py-0.5 rounded-full border border-orange-500/40">
                  {room.activeEvent.remainingSeconds}s
                </span>
              </div>
              <h3 className="font-serif text-sm font-black text-white">
                {room.activeEvent.event.name}
              </h3>
              <p className="text-xs font-serif text-zinc-300 line-clamp-2">
                {room.activeEvent.event.description}
              </p>
            </div>
          </div>
          {room.activeEvent.event.id === 'EV13' && room.activeEvent.speakerPlayerName && (
            <div className="px-3 py-1 rounded-xl bg-purple-950/90 border border-purple-400/50 text-purple-200 text-xs font-serif flex items-center gap-1.5 shrink-0">
              <Mic className="w-3.5 h-3.5 text-purple-300" />
              <span>Orador exclusivo: <strong>{room.activeEvent.speakerPlayerName}</strong></span>
            </div>
          )}
          {room.activeEvent.event.id === 'EV14' && (
            <div className="px-3 py-1 rounded-xl bg-indigo-950/90 border border-indigo-400/50 text-indigo-200 text-xs font-serif flex items-center gap-1.5 shrink-0">
              <Search className="w-3.5 h-3.5 text-indigo-300" />
              <span>Visibilidade: <strong>Apenas 3 primeiras evidências visíveis</strong></span>
            </div>
          )}
        </section>
      )}

      {/* 2. Analyst Inquiry (H04) Interactive Banner */}
      {room.analystInquiry && !room.analystInquiry.answered && (
        <section className="relative z-20 p-3 sm:p-4 rounded-2xl bg-gradient-to-r from-amber-950 via-black to-amber-950 border-2 border-amber-400/80 shadow-[0_0_25px_rgba(245,158,11,0.35)] animate-fade-in space-y-2.5">
          <div className="flex items-center justify-between gap-2 pb-1.5 border-b border-amber-500/30">
            <div className="flex items-center gap-2">
              <Search className="w-4 h-4 text-amber-400 animate-pulse" />
              <h3 className="text-xs sm:text-sm font-serif font-black text-amber-200 uppercase tracking-widest">
                INQUÉRITO DO ANALISTA (H04) • CONSULTA AO ORÁCULO
              </h3>
            </div>
            <span className="text-[10px] font-mono text-amber-300 bg-black/60 px-2.5 py-0.5 rounded-full border border-amber-500/50">
              Por: {room.analystInquiry.analystPlayerName}
            </span>
          </div>

          <p className="text-xs font-serif text-zinc-200 leading-relaxed">
            O Analista <strong>{room.analystInquiry.analystPlayerName}</strong> submeteu dois itens ao Oráculo:
            qual é <strong>mais verdadeiro</strong> para a solução do assassinato?
          </p>

          {isOracle || isPassAndPlayMode ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
              {room.analystInquiry.items.map((item, idx) => (
                <button
                  key={idx}
                  type="button"
                  onClick={() => {
                    soundEngine.playVictory();
                    if (onAnswerAnalystInquiry) {
                      onAnswerAnalystInquiry(item);
                    }
                  }}
                  className="py-2 px-3 rounded-xl bg-gradient-to-r from-amber-700 to-amber-600 hover:from-amber-600 hover:to-amber-500 border border-amber-300 text-white font-serif font-bold text-xs flex items-center justify-center gap-2 shadow-md transition-all active:scale-95 cursor-pointer"
                >
                  <CheckCircle2 className="w-4 h-4 text-amber-200 shrink-0" />
                  <span>Declarar &quot;{item}&quot; Mais Verdadeiro</span>
                </button>
              ))}
            </div>
          ) : (
            <div className="p-2 rounded-xl bg-black/50 border border-amber-500/30 text-[11px] font-serif text-amber-300/90 flex items-center gap-2">
              <Sparkles className="w-3.5 h-3.5 text-amber-400 shrink-0 animate-spin" />
              <span>Itens consultados: <strong>&quot;{room.analystInquiry.items.join('&quot; vs &quot;')}&quot;</strong>. Aguardando julgamento do Oráculo...</span>
            </div>
          )}
        </section>
      )}

      {/* 3. Interpreter Request (H10) Banner */}
      {room.interpreterRequest?.active && (
        <section className="relative z-20 p-3 rounded-2xl bg-gradient-to-r from-yellow-950/90 via-black to-amber-950/90 border border-yellow-400/70 shadow-lg flex items-center justify-between gap-3 animate-fade-in">
          <div className="flex items-center gap-2.5">
            <Sparkles className="w-5 h-5 text-yellow-400 animate-bounce shrink-0" />
            <div>
              <span className="text-[10px] font-mono text-yellow-300 font-bold uppercase tracking-wider block">
                INTÉRPRETE (H10) • FOCO CENTRAL
              </span>
              <p className="text-xs font-serif text-yellow-100">
                {isOracle || isPassAndPlayMode
                  ? 'O Intérprete solicitou que você utilize o Marcador Dourado na pista central do crime!'
                  : `${room.interpreterRequest.requesterPlayerName || 'O Intérprete'} solicitou o Marcador Dourado do Oráculo na evidência crucial.`}
              </p>
            </div>
          </div>
        </section>
      ...
      {activeFloatingModal === 'evidencias' && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex flex-col justify-between overflow-hidden animate-fade-in">
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-6 max-w-2xl mx-auto w-full flex flex-col gap-3">
            ...
            {/* EV14 Active Alert */}
            {room.activeEvent?.event?.id === 'EV14' && (
              <div className="p-2.5 rounded-xl bg-indigo-950/80 border border-indigo-500/50 flex items-center gap-2 text-indigo-200 text-xs font-serif shadow">
                <Flame className="w-4 h-4 text-indigo-400 shrink-0 animate-pulse" />
                <span>
                  <strong>EVENTO LUZ FRACA:</strong> A tempestade reduziu a visibilidade. Apenas as 3 primeiras evidências da mesa estão visíveis nesta rodada.
                </span>
              </div>
            )}

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 py-2">
              {filteredEvidences.map((ev, evIdx) => {
                const isDimLightActive = room.activeEvent?.event?.id === 'EV14';
                const isConcealedByDimLight = isDimLightActive && evIdx >= 3 && !isOracle && !isPassAndPlayMode;

                if (isConcealedByDimLight) {
                  return (
                    <div
                      key={ev.id}
                      className="aspect-[3/4] rounded-2xl p-2.5 border border-indigo-900/60 bg-black/90 flex flex-col items-center justify-center text-center relative overflow-hidden shadow-lg select-none"
                    >
                      <div className="absolute inset-0 bg-gradient-to-b from-indigo-950/40 via-black to-black opacity-80" />
                      <div className="relative z-10 flex flex-col items-center gap-1.5 p-2">
                        <div className="w-9 h-9 rounded-full bg-indigo-950/80 border border-indigo-500/40 flex items-center justify-center shadow">
                          <EyeOff className="w-4 h-4 text-indigo-400" />
                        </div>
                        <span className="text-[10px] font-serif font-black text-indigo-200 uppercase tracking-wider">
                          Luz Fraca
                        </span>
                        <span className="text-[8px] font-serif text-indigo-300/70 italic leading-tight">
                          Oculta na penumbra (apenas 3 visíveis)
                        </span>
                      </div>
                    </div>
                  );
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between border-b border-white/10 pb-1">
                      ...
                      {isDimLightActive && evIdx >= 3 && (isOracle || isPassAndPlayMode) && (
                        <span className="text-[7px] font-mono text-indigo-300 bg-indigo-950/80 px-1 py-0.5 rounded border border-indigo-500/30">
                          Oculto a Invest.
                        </span>
                      )}
                      <div className="flex items-center gap-1">
      ...
      {activeFloatingModal === 'chat' && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex flex-col justify-between p-3 sm:p-6 overflow-hidden animate-fade-in">
          <div className="max-w-xl mx-auto w-full flex-1 flex flex-col min-h-0">
            ...
            {/* Public vs Sussurro vs Bloco de Notas Tabs */}
            <div className="flex items-center justify-start sm:justify-center gap-1.5 sm:gap-2 p-1 bg-black/60 rounded-xl border border-white/10 my-2 shrink-0 overflow-x-auto no-scrollbar">
              <button
                onClick={() => {
                  soundEngine.playClick();
                  setChatTab('PUBLICO');
                }}
                className={`flex-1 min-w-[90px] py-1.5 text-xs font-serif font-bold uppercase tracking-wider rounded-lg transition-all shrink-0 whitespace-nowrap ${
                  chatTab === 'PUBLICO'
                    ? 'bg-red-900 text-white border border-red-500/60 shadow'
                    : 'text-zinc-400 hover:text-zinc-200'
                }`}
              ...
              <button
                ...
                className={`flex-1 min-w-[110px] py-1.5 text-xs font-serif font-bold uppercase tracking-wider rounded-lg transition-all flex items-center justify-center gap-1.5 shrink-0 whitespace-nowrap ${
                  chatTab === 'SUSSURRO'
              ...
              <button
                ...
                className={`flex-1 min-w-[130px] py-1.5 text-xs font-serif font-bold uppercase tracking-wider rounded-lg transition-all flex items-center justify-center gap-1.5 shrink-0 whitespace-nowrap ${
                  chatTab === 'NOTAS'
                    ? 'bg-gradient-to-r from-amber-900 to-amber-700 text-amber-100 border border-amber-400 shadow-[0_0_12px_rgba(245,158,11,0.4)]'
            ...
            {chatTab === 'NOTAS' ? (
              <div className="flex-1 flex flex-col min-h-0 bg-black/60 rounded-2xl border border-amber-600/40 p-3 sm:p-4 shadow-xl overflow-hidden">
                {/* Header with Title and Actions */}
                <div className="flex items-center justify-between pb-2 mb-2 border-b border-amber-900/40 shrink-0">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h3 className="text-xs font-serif font-black text-amber-200 uppercase tracking-wider">
                        BLOCO DE NOTAS DO INVESTIGADOR
                      </h3>
                      <span className="text-[9px] font-serif text-zinc-400">
                        Anotações pessoais e confidenciais • Salvo localmente
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5">
                    {chatNotesSavedNotice && (
                      <span className="text-[9px] font-mono text-emerald-400 flex items-center gap-1 animate-fade-in bg-emerald-950/70 border border-emerald-500/40 px-1.5 py-0.5 rounded">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        if (navigator.clipboard && chatNotesText) {
                          navigator.clipboard.writeText(chatNotesText);
                          soundEngine.playClick();
                          setChatNotesSavedNotice(true);
                          setTimeout(() => setChatNotesSavedNotice(false), 1500);
                        }
                      }}
                      className="px-2 py-1 rounded-lg bg-black/80 hover:bg-zinc-800 border border-white/10 text-[10px] font-serif text-zinc-300 transition-all active:scale-95"
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (window.confirm('Deseja limpar suas anotações?')) {
                          soundEngine.playClick();
                          handleUpdateChatNotes('');
                        }
                      }}
                      className="px-2 py-1 rounded-lg bg-red-950/40 hover:bg-red-900/60 border border-red-800/40 text-[10px] font-serif text-red-300 transition-all active:scale-95"
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-2 shrink-0">
                  <span className="text-[9px] font-mono text-zinc-500 shrink-0 uppercase">Atalhos:</span>
                  <button
                    type="button"
                    onClick={() => {
                      soundEngine.playClick();
                      const snippet = '\n✦ Suspeito: ';
                      handleUpdateChatNotes(chatNotesText ? `${chatNotesText}${snippet}` : '✦ Suspeito: ');
                    }}
                    className="px-2 py-0.5 rounded-full bg-amber-950/70 border border-amber-600/40 text-amber-200 text-[10px] font-serif hover:bg-amber-900/70 shrink-0 transition-all"
                  >
                    + Suspeito
                  </button>
                  <button
                    ...
                    onClick={() => {
                      soundEngine.playClick();
                      const snippet = '\n✦ Contradição: ';
                      handleUpdateChatNotes(chatNotesText ? `${chatNotesText}${snippet}` : '✦ Contradição: ');
                    }}
                    className="px-2 py-0.5 rounded-full bg-amber-950/70 border border-amber-600/40 text-amber-200 text-[10px] font-serif hover:bg-amber-900/70 shrink-0 transition-all"
                  >
                    + Contradição
                  ...
                  <button
                    ...
                    onClick={() => {
                      soundEngine.playClick();
                      const snippet = '\n✦ Método provável: ';
                      handleUpdateChatNotes(chatNotesText ? `${chatNotesText}${snippet}` : '✦ Método provável: ');
                    }}
                    className="px-2 py-0.5 rounded-full bg-red-950/70 border border-red-600/40 text-red-200 text-[10px] font-serif hover:bg-red-900/70 shrink-0 transition-all"
                  >
                    + Método
                  </button>
                  <button
                    ...
                    onClick={() => {
                      soundEngine.playClick();
                      const snippet = '\n✦ Objeto-chave: ';
                      handleUpdateChatNotes(chatNotesText ? `${chatNotesText}${snippet}` : '✦ Objeto-chave: ');
                    }}
                    className="px-2 py-0.5 rounded-full bg-blue-950/70 border border-blue-600/40 text-blue-200 text-[10px] font-serif hover:bg-blue-900/70 shrink-0 transition-all"
                  >
                    + Objeto
                  </button>
                </div>
                ...
                <div className="flex-1 min-h-0 flex flex-col">
                  <textarea
                    value={chatNotesText}
                    onChange={(e) => handleUpdateChatNotes(e.target.value)}
                    placeholder="Escreva aqui suas suspeitas, deduções, alibis questionáveis e pistas eliminadas durante o interrogatório... Seu texto é salvo automaticamente neste aparelho."
                    className="flex-1 w-full p-3 sm:p-4 rounded-xl bg-black/85 border border-amber-600/30 text-xs sm:text-sm font-serif text-amber-100 placeholder-zinc-600 focus:outline-none focus:border-amber-400/80 resize-none leading-relaxed overflow-y-auto no-scrollbar shadow-inner"
                  />
                </div>

                {/* Footer Bar */}
                <div className="pt-2 flex items-center justify-between text-[10px] font-serif text-zinc-400 shrink-0 border-t border-white/5 mt-2">
                  <span>{chatNotesText.length} caracteres • Salvo no navegador</span>
                  <button
                    type="button"
                    onClick={() => {
                      soundEngine.playClick();
                    ...
                    className="text-amber-400 hover:text-amber-200 underline font-serif flex items-center gap-1"
                  >
                    <BookOpen className="w-3 h-3" />
                    <span>Ver Dossiê e Pistas Eliminadas</span>
                  </button>
                </div>
              </div>
            ) : (
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between gap-1">
                                ...
                                <span className="text-[9px] font-mono text-zinc-500 shrink-0">
                                  {typeof msg.timestamp === 'string' && msg.timestamp.includes(':') && !msg.timestamp.includes('T')
                                    ? msg.timestamp
            ...
            {chatTab !== 'NOTAS' && (chatTab !== 'SUSSURRO' || isConspirator || isPassAndPlayMode) && (
              <div className="mt-2 shrink-0 flex flex-col gap-1.5">
                {/* Quick Notebook shortcut link above chat input */}
                <div className="flex items-center justify-between px-1 text-[10px] font-serif">
                  <button
                    type="button"
                    onClick={() => {
                      soundEngine.playClick();
                    ...
                    className="flex items-center gap-1 text-amber-300/90 hover:text-amber-100 transition-colors"
                    title="Abrir Bloco de Notas do Investigador"
                  >
                    <NotebookPen className="w-3 h-3 text-amber-400" />
                    <span>
                      Bloco de Notas do Investigador{' '}
                      <strong className="text-amber-200">
                        {chatNotesText ? `(${chatNotesText.trim().slice(0, 20)}...)` : '(Escrever notas)'}
                      </strong>
                    </span>
                  </button>
                  <span className="text-[9px] font-mono text-zinc-500">
                    {chatTab === 'PUBLICO' ? 'Chat Público' : 'Canal Secreto'}
                  </span>
                </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex flex-col justify-between overflow-hidden animate-fade-in">
          ...
          >
            ...
            >
              ...
              <p className="text-xs sm:text-sm font-serif text-zinc-200 leading-relaxed max-w-md italic px-2 whitespace-pre-line">
                "{defaultStory}"
              </p>
            ...
            <div className="relative z-10 w-full flex-1 py-4 sm:py-8 pl-2 sm:pl-6 pr-18 sm:pr-24 flex flex-col justify-around gap-3 sm:gap-6 overflow-y-auto no-scrollbar">
              {(() => {
                ...
                return rows.map((rowPlayers, rIdx) => (
                  <div
                    key={`fs-row-${rIdx}`}
                    className="flex items-center justify-center gap-1.5 sm:gap-4 flex-wrap sm:flex-nowrap w-full px-1 sm:px-2 py-1"
                  >
                    {rowPlayers.map((player) => {
                      const seatNum = players.findIndex((p) => p.id === player.id) + 1;
                      const isSelected = selectedPlayerForDossier?.id === player.id && activeFloatingModal === 'cartas';
      ...
      {/* INVESTIGATOR NOTEBOOK MODAL (CADERNO DE ANOTAÇÕES) */}
      {showNotebookModal && (
        <InvestigatorNotebookModal
