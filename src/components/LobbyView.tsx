...
export const LobbyView: React.FC<LobbyViewProps> = ({
  ...
  const myPlayer = room.players.find((p) => p.id === myPlayerId) || room.players.find((p) => !p.isAI) || room.players[0];
  const isHost = myPlayer?.isHost || room.players[0]?.id === myPlayerId || (myPlayer && room.players[0]?.id === myPlayer.id);
  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_rgba(69,26,3,0.3)_0%,_rgba(0,0,0,0.5)_50%,_rgba(0,0,0,0.9)_100%)]" />
        <AtmosphericDust density={20} />
      </div>

      {/* Main Card Container - Exact Mobile Frame with Single-Screen Layout & Mobile Scrollability */}
      <div
        id="lobby-main-card"
        className="relative z-10 w-full max-w-sm sm:max-w-md min-h-[400px] sm:min-h-[480px] max-h-[96dvh] sm:max-h-[92vh] flex flex-col justify-between p-3 sm:p-4 rounded-2xl sm:rounded-3xl border-2 border-amber-600/60 bg-black/90 shadow-[0_0_50px_rgba(0,0,0,0.95)] overflow-hidden bg-cover bg-center my-auto"
      ...
      >
          ...
          <div className="flex items-end justify-between mt-2 pt-1 border-b border-amber-900/35 pb-2">
            ...
            <div className="text-right">
              ...
              <div className="flex items-center gap-1.5 justify-end mt-0.5">
                {room.isPrivate && (
                  <span className="inline-flex items-center gap-1 text-[8.5px] font-mono text-amber-300 bg-red-950/80 px-1.5 py-0.5 rounded border border-red-500/40">
                    <Lock className="w-2.5 h-2.5 text-amber-400" />
                    <span>PRIVADA</span>
                    {room.password && <span className="text-zinc-300">({room.password})</span>}
                  </span>
                )}
                {room.gameMode && (
        ...
        >
          ...
          {(room?.players || []).map((player) => {
            ...
            let displayName = player.name || 'Investigador';
