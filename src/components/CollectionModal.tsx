...
export const CollectionModal: React.FC<CollectionModalProps> = ({
          ...
          try {
            ...
            if (match.category === 'personagens' || match.category === 'brasoes') {
              ...
              if (charObj && charObj.number) {
                const padSlot = (charObj.number - 1).toString().padStart(2, '0');
                const padNum = charObj.number.toString().padStart(2, '0');
                matchedEntries[`perso_${charObj.number}`] = dataUrl;
                matchedEntries[`perso${charObj.number}`] = dataUrl;
                matchedEntries[`perso_${padNum}`] = dataUrl;
                matchedEntries[`personagem_${padNum}`] = dataUrl;
                matchedEntries[`char_${padSlot}`] = dataUrl;
              }
            ...
            } else if (match.category === 'evidencias') {
              const num = parseInt(match.cardId.replace(/\D/g, ''), 10);
              const pad = num.toString().padStart(2, '0');
              matchedEntries[match.cardId] = dataUrl;
              matchedEntries[match.cardId.toLowerCase()] = dataUrl;
              matchedEntries[`evidencia_${match.cardId}`] = dataUrl;
              matchedEntries[`evidencia_${match.cardId.toLowerCase()}`] = dataUrl;
              matchedEntries[`evidencia${match.cardId}`] = dataUrl;
              matchedEntries[`evidencia${match.cardId.toLowerCase()}`] = dataUrl;
              matchedEntries[`evidencia_${pad}`] = dataUrl;
              matchedEntries[`evidencia${pad}`] = dataUrl;
              matchedEntries[`evidencia_${num}`] = dataUrl;
            }
  ...
  const charactersCountLinked = CHARACTERS.filter((c) => {
    const padSlot = c.number ? (c.number - 1).toString().padStart(2, '0') : '';
    const padNum = c.number ? c.number.toString().padStart(2, '0') : '';
    return !!(
      customArts[`char_card_${c.id}`] ||
      customArts[c.id] ||
      customArts[`crest_${c.id}`] ||
      (c.number && (
        customArts[`perso_${c.number}`] ||
        customArts[`perso${c.number}`] ||
        customArts[`perso_${padNum}`] ||
        customArts[`personagem_${padNum}`] ||
        customArts[`char_${padSlot}`]
      ))
    );
  }).length;
          ...
          {category === 'lote' && (
            <div className="max-w-4xl mx-auto py-3 px-2 space-y-5 animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
                {/* Evidências status */}
                <div className="p-3.5 rounded-2xl bg-black/60 border border-sky-950/80 flex flex-col justify-between space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-serif font-bold text-sky-200 text-xs flex items-center gap-1.5">
                      <FileSearch className="w-4 h-4 text-sky-400" /> 60 Evidências
                    </span>
                    <span className="font-mono text-xs font-bold text-sky-300">
                      {evidencesCountLinked}/60
                    </span>
                  </div>
                  <div className="w-full bg-zinc-900 h-1.5 rounded-full overflow-hidden">
                    <div
                      className="bg-sky-500 h-full transition-all"
                      style={{ width: `${(evidencesCountLinked / 60) * 100}%` }}
                    />
                  </div>
                  <span className="text-[10px] font-mono text-zinc-400">
                    {evidencesCountLinked === 60 ? '✅ Todas as 60 vinculadas' : `Faltam ${60 - evidencesCountLinked} (evidencia_E01..E60)`}
                  </span>
                </div>
                ...
                {/* Eventos status */}
                <div className="p-3.5 rounded-2xl bg-black/60 border border-emerald-950/60 flex flex-col justify-between space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-serif font-bold text-emerald-200 text-xs flex items-center gap-1.5">
                      <Flame className="w-4 h-4 text-emerald-400" /> 16 Eventos (EV01-EV16)
                    </span>
                    <span className="font-mono text-xs font-bold text-emerald-300">
                      {eventsCountLinked}/16
                    </span>
                  </div>
                  <div className="w-full bg-zinc-900 h-1.5 rounded-full overflow-hidden">
                    <div
                      className="bg-emerald-500 h-full transition-all"
                      style={{ width: `${(eventsCountLinked / 16) * 100}%` }}
                    />
                  </div>
                  <span className="text-[10px] font-mono text-zinc-400">
                    {eventsCountLinked === 16 ? '✅ Todos os 16 vinculados' : `Faltam ${16 - eventsCountLinked} arquivos EVXX.png`}
                  </span>
                </div>
              ...
              <div className="p-4 sm:p-5 rounded-2xl bg-black/60 border border-amber-500/30 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs font-mono text-zinc-300">
                  <div className="p-2.5 rounded-xl bg-zinc-950/80 border border-sky-500/30 space-y-1">
                    <span className="font-bold text-sky-300 block">🔍 Evidências (E01 a E60):</span>
                    <p className="text-[11px] text-zinc-400">
                      Reconhece: <code className="text-sky-300 font-bold">evidencia_E01.png</code> até <code className="text-sky-300 font-bold">evidencia_E60.png</code>, <code className="text-amber-200">E01.png</code>, <code className="text-amber-200">evidencia_01.png</code> ou título
                    </p>
                  </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-zinc-950/80 border border-emerald-500/20 space-y-1">
                    <span className="font-bold text-emerald-300 block">⚡ Eventos (EV01 a EV16):</span>
                    <p className="text-[11px] text-zinc-400">
                      Formatos aceitos: <code className="text-amber-200">EV01.png</code> a <code className="text-amber-200">EV16.png</code>, <code className="text-amber-200">evento_01.png</code> ou nome do evento
                    </p>
                  </div>
                  <div className="p-2.5 rounded-xl bg-zinc-950/80 border border-white/5 space-y-1">
