...
import { generateDynamicCrimeNarrative } from './crimeNarrativeEngine';

export { generateDynamicCrimeNarrative };
...
export function finishOraclePhase(room: RoomState): RoomState {
  ...
  if (updated.settings.allowEvents && updated.round >= 2 && !updated.activeEvent) {
    ...
    const isWhisper = randomEvent.id === 'EV13' || randomEvent.name.toLowerCase().includes('sussurro');
    const investigators = updated.players.filter((p) => p.role !== 'oraculo');
    const speaker = investigators[Math.floor(Math.random() * investigators.length)] || updated.players[0];

    updated.activeEvent = {
      event: randomEvent,
      remainingSeconds: randomEvent.duration || 60,
      speakerPlayerId: isWhisper ? speaker?.id : undefined,
      speakerPlayerName: isWhisper ? speaker?.name : undefined,
    };
    ...
    updated.logs.push({
      ...
      text: `⚠️ EVENTO DESENCADEADO: [${randomEvent.name}] - ${randomEvent.effect}${
        isWhisper && speaker ? ` [Orador Exclusivo: ${speaker.name}]` : ''
      }`,
      type: 'event',
    });
...
export function handleAbilityUse(
  ...
  if (abilityId === 'H01') {
    ...
    if (!targetEv.markedColor) {
      targetEv.markedColor = 'vermelho';
    }
    updated.logs.push({
      id: `log_${Date.now()}`,
      timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ...
    const items = extraPayload?.itemNames && extraPayload.itemNames.length >= 2
      ? [extraPayload.itemNames[0].trim(), extraPayload.itemNames[1].trim()]
      : extraPayload?.itemNames?.length === 1
      ? [extraPayload.itemNames[0].trim(), 'Outra Pista']
      : ['Pista Alfa', 'Pista Beta'];

    updated.analystInquiry = {
      analystPlayerId: player.id,
      analystPlayerName: player.name,
      items,
      answered: false,
    };

    const inquiryText = `🔍 ${player.name} [Analista] realizou um inquérito formal ao Oráculo: Entre "${items[0]}" e "${items[1]}", qual é mais verdadeiro para a resolução do crime?`;
    updated.logs.push({
      id: `log_${Date.now()}`,
      timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      text: inquiryText,
    ...
    updated.messages = [
      ...(updated.messages || []),
      {
        id: `msg_analyst_inquiry_${Date.now()}`,
        senderId: player.id,
        senderName: `${player.name} [Analista]`,
        senderRole: 'investigador',
        text: `🔍 INQUÉRITO AO ORÁCULO: Qual destes elementos é mais verdadeiro: "${items[0]}" OU "${items[1]}"?`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        isSystem: true,
      },
    ];
    ...
    updated.interpreterRequest = {
      active: true,
      requesterPlayerId: player.id,
      requesterPlayerName: player.name,
    };
    const interpreterText = `🌟 ${player.name} [Intérprete] invocou o Selo Dourado! O Oráculo foi convocado a posicionar um Marcador Dourado na pista central do mistério!`;
    updated.logs.push({
      id: `log_${Date.now()}`,
      timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ...
    updated.messages = [
      ...(updated.messages || []),
      {
        id: `msg_interpreter_${Date.now()}`,
        senderId: player.id,
        senderName: `${player.name} [Intérprete]`,
        senderRole: 'investigador',
        text: `🌟 ATIVAÇÃO DO INTÉRPRETE: Convocou o Oráculo a utilizar obrigatoriamente o MARCADOR DOURADO na pista central do crime!`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        isSystem: true,
      },
    ];
...
export function handleAnswerAnalystInquiry(
  room: RoomState,
  selectedItem: string,
  oraclePlayerId?: string
): RoomState {
  const updated = { ...room };
  if (!updated.analystInquiry) return updated;

  const prevInquiry = updated.analystInquiry;
  updated.analystInquiry = {
    ...prevInquiry,
    selectedItem,
    answered: true,
  ...
  const oracle =
    updated.players.find((p) => p.role === 'oraculo') ||
    updated.players.find((p) => p.id === oraclePlayerId);
  const oracleName = oracle ? oracle.name : 'O Oráculo';

  const announcement = `🔮 RESPOSTA DO ORÁCULO AO ANALISTA [${prevInquiry.analystPlayerName}]: Entre as opções analisadas, o elemento mais verdadeiro na cena do crime é: "${selectedItem}"!`;
  ...
  updated.messages = [
    ...(updated.messages || []),
    {
      id: `msg_oracle_analyst_${Date.now()}`,
      senderId: oracle?.id || 'oracle',
      senderName: `${oracleName} (Oráculo)`,
      senderRole: 'oraculo',
      text: announcement,
      timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      isSystem: true,
  ...
  const sanitized: RoomState = {
    ...
    players: room.players.map((p) => {
      ...
      if (!isGameOver && p.id !== playerId) {
        // ORACLE is always visible to EVERYONE
        if (p.role === 'oraculo') {
          visibleRole = 'oraculo';
        }
        // KILLER and ACCOMPLICE see each other
        else if (isKiller && p.role === 'cumplice') {
          visibleRole = 'cumplice';
        } else if (isAccomplice && p.role === 'assassino') {
          visibleRole = 'assassino';
        }
        // ORACLE sees all special roles (Assassin, Accomplice, Saboteur)
        else if (isOracle && (p.role === 'assassino' || p.role === 'cumplice' || p.role === 'sabotador')) {
          visibleRole = p.role;
        }
        // Otherwise, role remains hidden
        else {
...
export function handleDrawRandomEvent(room: RoomState): RoomState {
  ...
  const randomEvent = EVENTS[Math.floor(Math.random() * EVENTS.length)];
  const isWhisper = randomEvent.id === 'EV13' || randomEvent.name.toLowerCase().includes('sussurro');
  const nonOraclePlayers = updated.players.filter((p) => p.role !== 'oraculo');
  const chosenSpeaker = nonOraclePlayers.length > 0
    ? nonOraclePlayers[Math.floor(Math.random() * nonOraclePlayers.length)]
    : updated.players[0];

  updated.activeEvent = {
    event: randomEvent,
    remainingSeconds: randomEvent.duration || 90,
    speakerPlayerId: isWhisper ? chosenSpeaker?.id : undefined,
    speakerPlayerName: isWhisper ? chosenSpeaker?.name : undefined,
  };
  updated.eventsActivatedCount = (updated.eventsActivatedCount || 0) + 1;
  ...
  updated.logs.push({
    ...
    text: `⚡ EVENTO DA BIBLIOTECA INVOCADO: [${randomEvent.name}] - ${randomEvent.effect}${
      isWhisper && chosenSpeaker ? ` [Orador Exclusivo: ${chosenSpeaker.name}]` : ''
    }`,
    type: 'system',
  });

  if (isWhisper && chosenSpeaker) {
    updated.messages = [
      ...(updated.messages || []),
      {
        id: `msg_whisper_${Date.now()}`,
        senderId: 'system',
        senderName: 'Códice Sombrio',
        senderRole: 'oraculo',
        text: `🤫 EVENTO SUSSURRO: Apenas [${chosenSpeaker.name}] tem a palavra! Todos os outros investigadores estão sob voto de silêncio (chat e voz bloqueados).`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        isSystem: true,
      },
    ];
...
export function generateAIBotPersonalResponse(
  room: RoomState,
  senderPlayerId: string,
  userMessageText: string
): { bot: Player; replyText: string } | null {
  const aiBots = room.players.filter((p) => p.isAI && p.id !== senderPlayerId);
  if (aiBots.length === 0) return null;

  const sender = room.players.find((p) => p.id === senderPlayerId);
  const senderName = sender?.name || 'Investigador';

  // Pick an AI bot to reply
  const bot = aiBots[Math.floor(Math.random() * aiBots.length)];

  const lowerMsg = userMessageText.toLowerCase();
  const markedEvidences = room.evidencesOnTable.filter((e) => e.markedOptionIndex !== undefined);
  const otherSuspects = room.players.filter((p) => p.id !== bot.id && p.id !== senderPlayerId && p.role !== 'oraculo');
  const targetSuspect = otherSuspects.length > 0 ? otherSuspects[Math.floor(Math.random() * otherSuspects.length)] : null;
  const clue = markedEvidences.length > 0 ? markedEvidences[Math.floor(Math.random() * markedEvidences.length)] : null;
  const clueOption = clue && clue.markedOptionIndex !== undefined ? clue.options[clue.markedOptionIndex] : null;

  let replyText = '';

  // 1. If player directly questioned or accused the bot
  const botFirstName = bot.name.split(' ')[0].toLowerCase();
  if (
    lowerMsg.includes(botFirstName) ||
    lowerMsg.includes('você') ||
    lowerMsg.includes('voce') ||
    lowerMsg.includes('culpado') ||
    lowerMsg.includes('assassino')
  ) {
    if (bot.role === 'assassino') {
      const deflections = [
        `@${senderName}, entendo sua desconfiança, mas você está caindo em um blefe! Observe as cartas de ${targetSuspect?.name || 'outros suspeitos'}... aquilo sim não encaixa com as pistas do Oráculo.`,
        `@${senderName}, me acusar agora só favorece o verdadeiro assassino. Minha consciência e minhas cartas estão limpas perante o Códice. O que me diz das provas de ${targetSuspect?.name || 'outro presente'}?`,
        `@${senderName}, manter a calma é a maior virtude de um detetive. Se você analisar a pista "${clueOption || 'destacada'}", verá que não há correlação com meus pertences.`,
      ];
      replyText = deflections[Math.floor(Math.random() * deflections.length)];
    } else {
      const innocentReplies = [
        `@${senderName}, tem minha palavra solene: minhas mãos estão limpas de sangue. Em vez de suspeitar de mim, repare na marcação que o Oráculo fez em [${clue?.title || 'Códice'}].`,
        `@${senderName}, compreendo sua ansiedade com o relógio correndo, mas sou um aliado da justiça. Devemos concentrar nossa atenção nas cartas de ${targetSuspect?.name || 'outros nobres'}.`,
        `@${senderName}, se eu fosse o culpado, não estaria instigando todos a examinarem o Códice com tanto rigor. Vamos focar nos fatos!`,
      ];
      replyText = innocentReplies[Math.floor(Math.random() * innocentReplies.length)];
    }
  }
  // 2. If player asks about suspects, evidence or theories
  else if (
    lowerMsg.includes('quem') ||
    lowerMsg.includes('acha') ||
    lowerMsg.includes('suspeito') ||
    lowerMsg.includes('pista') ||
    lowerMsg.includes('oraculo') ||
    lowerMsg.includes('oráculo') ||
    lowerMsg.includes('método') ||
    lowerMsg.includes('objeto')
  ) {
    if (bot.role === 'assassino' && targetSuspect) {
      const killerDeceptions = [
        `@${senderName}, estava justamente ponderando sobre isso. Olhe a mesa de ${targetSuspect.name}. O que ele(a) tem a dizer sobre [${targetSuspect.methods[0]?.name || 'aquele método'}]?`,
        `@${senderName}, se o Oráculo destacou "${clueOption || 'aquele indício'}", parece um encaixe quase evidente com ${targetSuspect.name}. Não podemos hesitar.`,
        `@${senderName}, concordo com você. Para mim, ${targetSuspect.name} está demonstrando um nervosismo bastante suspeito nesta rodada.`,
      ];
      replyText = killerDeceptions[Math.floor(Math.random() * killerDeceptions.length)];
    } else {
      const investigatorTheories = [
        `@${senderName}, excelente dedução! Ao cruzar "${clueOption || 'a marcação'}" de [${clue?.title || 'Evidência'}] com as cartas expostas, ${targetSuspect?.name || 'um dos suspeitos'} desponta como forte suspeito.`,
        `@${senderName}, meu raciocínio converge com o seu. Se descartarmos o impossível, o que sobra é a verdade. Notou o objeto sobre a mesa de ${targetSuspect?.name || 'um investigador'}?`,
        `@${senderName}, o Oráculo nos deu uma direção precisa. Acredito que devemos inquirir tanto ${targetSuspect?.name || 'o suspeito da ponta'} quanto quem ainda não se pronunciou abertamente.`,
      ];
      replyText = investigatorTheories[Math.floor(Math.random() * investigatorTheories.length)];
    }
  }
  // 3. Conversational personal reply (ChatGPT style)
  else {
    const generalChat = [
      `@${senderName}, li com atenção suas palavras. Com o avançar da Rodada ${room.round}, cada frase dita nesta sala pode nos aproximar da verdade ou nos afundar em engano.`,
      `@${senderName}, você trouxe uma perspectiva astuta. Minha leitura das gemas do Oráculo me diz que teremos a resposta antes da última badalada.`,
      `@${senderName}, concordo em gênero e número! O silêncio do Oráculo fala mais alto que qualquer desculpa. Vamos examinar as cartas de ${targetSuspect?.name || 'todos'} com atenção redobrada.`,
      `@${senderName}, enquanto você falava, notei que ${targetSuspect?.name || 'outro nobre'} desviou o olhar. O que acha de fazermos uma votação preliminar?`,
    ];
    replyText = generalChat[Math.floor(Math.random() * generalChat.length)];
  }
